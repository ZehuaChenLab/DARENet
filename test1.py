...
# from networks.lmz import Baseline_Mixer_Dskip
...
def test_ce_net_vessel():
    ...
    model_dict = {'UNet': Unet, 'DLinkNet': DLinkNet, 'NLlinkNet': NLlinkNet, 'DeepLabv3_plus': DeepLabv3_plus,
                ...
                'Baseline': Baseline, 'Baseline_Mixer_Dskip': Baseline_Mixer_Dskip,'Baseline_Ex':Baseline_Ex,'CMTFNet':CMTFNet,'CARENet':CARENet,
                  'NewNet':NewNet,'MobileRoadNet':MobileRoadNet,'LID2Mamba':LID2Mamba,'CPSSNet':CPSSNet}
    ...
    # 0.2188694680230223 
    solver.load('/home/smart/Road/weights/best_weights/'+ model_name+ '_' + 'spacenet'+'_0.2252763313033598_best_model.pth')
    # solver.load('/home/smart/Road/weights/best_weights/LID2Mamba_spacenet_best_model.pth')
    # target = '/data/guoxuejun/yangjialin/CE-Net/submits/log_U_Net/'
    target = './submits/'+ model_name+'/spacenet/'
    ...
    mylog = Logger('./logs/test/'+model_name+'_spacenet' + '.log')
    ...
    mylog.write('************************' + '\n')
    mylog.write('Accuracy:    {}'.format(np.mean(total_acc), np.std(total_acc)) + '\n')
    mylog.write('Precision:   {}'.format(np.mean(total_pre), np.std(total_pre)) + '\n')
    mylog.write('Recall:      {}'.format(np.mean(total_sen), np.std(total_sen))+ '\n')
    mylog.write('F1_Score:    {}'.format(np.mean(total_f1), np.std(total_f1)) + '\n')
    mylog.write('IoU_Score:   {}'.format(np.mean(total_iou), np.std(total_iou))+ '\n')
