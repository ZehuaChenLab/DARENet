...
class RCFSNet(nn.Module):
    def __init__(self, img_size=1024, num_classes=1, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,
                 base_channel=32):
    ...
    def forward(self, inputs):
        if self.img_size != 1024:
            inputs = F.interpolate(inputs, size=1024, mode='bilinear', align_corners=True)
        ...
        main_out = self.main_head(d1 + self.conv5(self.hd5_d1(hd5)) + self.conv4(self.hd4_d1(d4)) + self.conv3(
                self.hd3_d1(d3)) + self.conv2(self.hd2_d1(d2)))
        if self.img_size != 1024:
            main_out = F.interpolate(main_out, size=self.img_size, mode='bilinear', align_corners=True)
        return main_out
