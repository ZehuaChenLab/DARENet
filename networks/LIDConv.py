...
class LID2Conv(nn.Module):
    def __init__(self, dim, num_param, stride=1):
        ...
        self.p_conv = nn.Conv2d(dim, 2 * num_param, kernel_size=3, padding=1, stride=stride)
        self.p_conv = StripDynamicConv2d(dim, 2 * num_param, padding=1, stride=stride)
    ...
    def _reshape_x_offset(x_offset, num_param):
        ...
        # then Conv3d(c,c_out, kernel_size =(num_param,1,1),stride=(num_param,1,1),bias= False)

        # using 1 × 1 Conv
        # x_offset = x_offset.permute(0,1,4,2,3),
        # then, x_offset.view(b,c×num_param,h,w)
        # finally, Conv2d(c×num_param,c_out, kernel_size =1,stride=1,bias= False)

        # using the column conv as follow，
        # then, Conv2d(inc, outc, kernel_size=(num_param, 1), stride=(num_param, 1), bias=bias)
        x_offset = rearrange(x_offset, 'b c h w n -> b c (h n) w')
        return x_offset


class StripDynamicConv2d(nn.Module):
    def __init__(self, dim, num_param, reduction_ratio=4, dilation=1, num_groups=4, bias=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.bias is not None:
            ...
            bias = None
        x = F.conv2d(x.reshape(1, -1, H, W), weight=weight, bias=bias, stride=self.stride,
                     padding=padding, dilation=self.dilation, groups=B * C)
