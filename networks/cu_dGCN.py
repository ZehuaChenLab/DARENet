...
class ConvNeXtBlock(nn.Module):
    def __init__(self, FDim, Expansion, DropPathRate, LayerScaleInitial, Training):
        super().__init__()
        self.gamma = nn.Parameter(torch.ones((FDim)) * LayerScaleInitial,
                                  requires_grad=True) if LayerScaleInitial > 0 else None
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_Channels, Depths, FeatureDimensions, StochasticDepthRate, LayerScaleInitial, Training):
        ...
        dropIdx = 0
        for i in range(
                len(FeatureDimensions)):  # 4 Stages (one for each FeatureDimension) with multiple residual blocks of given respective depths.
...
class CBA_UP128(nn.Sequential):
    def __init__(self, in_Channels, out_Channels, groups=1):
        super().__init__(nn.Conv2d(in_Channels, in_Channels // 4, 1, groups=groups),
                         ...
                         nn.ReLU(True),
                         nn.ConvTranspose2d(in_Channels // 4, in_Channels // 4, 3, stride=2, padding=1,
...
class GraphReasoning(nn.Module):
    ...
    def forward(self, r_o64, r_o128, r_o257):
        ...
        o_gcn_128_to_257 = F.interpolate(o_gcn_128_and_64, size=(o_gcn_257.shape[2], o_gcn_257.shape[3]),
                                         mode="bilinear")
...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in reversed(range(len(features) - 1)):
            ...
            FPN_feature_level = feature + F.interpolate(FPN_feature_level, size=feature.shape[-2:], mode='bilinear',
                                                        align_corners=False)
...
class ConvNeXt_UPerNet_DGCN_MTL(nn.Module):
    def __init__(self, ModelArch="Base"):
        ...
        Depths, FeatureDimensions, StochasticDepthRate, LayerScaleInitial = ConvNeXt_Archs[
            ModelArch]  # 'Base': [[3, 3, 27, 3], [128, 256, 512, 1024], 0.4, 1.0],

        self.encoder = Encoder(3, Depths, FeatureDimensions, StochasticDepthRate, LayerScaleInitial,
                               Training=self.training)
        self.decoder = Decoder(FeatureDimensions, FPN_Dimension=FeatureDimensions[0], tasks_classes=[2, 37],
                               scales=(1, 2, 3, 6))  # instead of 1,2,3,6 (arcgis (1, 3, 5, 6))
