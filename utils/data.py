...
def default_load(id, dataset, root):
    if dataset == 'ROAD':
        img_suf = '_sat.jpg'
        mask_suf = '_mask.png'
    elif dataset == 'Mas':
        img_suf = '_sat.tiff'
        mask_suf = '_mask.tif'
    elif dataset == 'CHN6-CUG':
        img_suf = '_sat.jpg'
        mask_suf = '_mask.png'
    elif dataset == 'spacenet':
        img_suf = '_sat.png'
        mask_suf = '_mask.png'
    img = cv2.imread(os.path.join(root, '{}'+img_suf).format(id))
    mask = cv2.imread(os.path.join(root + '{}'+mask_suf).format(id), cv2.IMREAD_GRAYSCALE)
    mask[mask > 0] = 255
...
class ImageFolder(data.Dataset):
    def __init__(self, root):
        self.dataset = root.split('/')[-3]
        imagelist = list(filter(lambda x: x.find('sat') != -1, os.listdir(root)))
        if self.dataset == 'ROAD':
            self.ids = list(map(lambda x: x[:-8], imagelist))
        elif self.dataset == 'Mas':
            self.ids = list(map(lambda x: x[:-9], imagelist))
        elif self.dataset == 'CHN6-CUG':
            self.ids = list(map(lambda x: x[:-8], imagelist))
    ...
    def __getitem__(self, index):
        id = self.ids[index]
        img, mask = self.load(id, self.dataset, self.root)
