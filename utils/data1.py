...
def read_Parisroad_datasets(root_path, mode='train'):
    ...
    image_root = os.path.join(root_path, 'image/train/')
    gt_root = os.path.join(root_path, 'mask1/train/')
    for image_name in os.listdir(image_root):
...
def read_Parisbuild_datasets(root_path, mode='train'):
    images = []
    masks = []

    image_root = os.path.join(root_path, 'image/train/')
    gt_root = os.path.join(root_path, 'mask2/train/')
    for image_name in os.listdir(image_root):
        image_path = os.path.join(image_root, image_name.split('.')[0] + '.png')
        label_path = os.path.join(gt_root, image_name.split('.')[0].replace('image','labels')+'.jpg')
...
def read_Space_datasets(root_path, mode='train'):
    images = []
    masks = []
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root_path, datasets='Messidor',  mode='train'):
        ...
        assert self.dataset in ['RIM-ONE', 'Messidor', 'ORIGA', 'DRIVE', 'Cell', 'Vessel','Mas', 'CHG','spacenet','paris_road','paris_build'], \
        ...
        elif self.dataset == 'spacenet':
            self.images, self.labels = read_Space_datasets(self.root, self.mode)
        elif self.dataset == 'paris_road':
            self.images, self.labels = read_Parisroad_datasets(self.root, self.mode)
        elif self.dataset == 'paris_build':
            self.images, self.labels = read_Parisbuild_datasets(self.root, self.mode)
        else:
